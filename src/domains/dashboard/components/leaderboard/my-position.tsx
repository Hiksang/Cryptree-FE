  ...
  return (
    <div className="bg-gradient-to-r from-brand/10 to-brand/5 border border-brand/20 rounded-[8px] p-6">
      <div className="flex items-center gap-3 mb-3">
        ...
        <h3 className="text-[16px] font-semibold text-text-primary">{t.dashboard.leaderboard.myRank}</h3>
      </div>

      <div className="grid grid-cols-3 gap-4">
        <div>
          <p className="text-[12px] text-text-muted mb-1">{t.dashboard.leaderboard.colRank}</p>
          <p className="text-[24px] font-bold text-brand tabular-nums">
            #{position.rank}
          </p>
        </div>
        <div>
          <p className="text-[12px] text-text-muted mb-1">{t.dashboard.leaderboard.colTier}</p>
          <p className="text-[16px] font-semibold" style={{ color: tier.color }}>
            {tier.icon} {tier.label}
          </p>
        </div>
        <div>
          <p className="text-[12px] text-text-muted mb-1">{t.dashboard.leaderboard.value}</p>
